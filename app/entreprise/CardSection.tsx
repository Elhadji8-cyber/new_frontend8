...
export const CardSecion = function(){
    return(
        <section className="flex flex-col mb-5 p-5 md:p-6 lg:p-20 justify-center items-center mt-20 gap-3">
         <h1 className="text-xl md:text-4xl lg:text-5xl font-bold p-1">Pourquoi Choisir GénieFormation ?</h1>
         <div className="grid grid-cols-1 justify-items-center gap-3 md:grid-cols-2 lg:grid-cols-3 mt-3">
            {CardItem.map((d, indx) => (
                <motion.div key={indx}
                ...
                hover:shadow-xl transition-shadow duration-300 bg-neutral-100"
                >
                  {(()=> {
                    const Icon = d.icon;
                    return <Icon className="text-5xl text-white p-2 rounded-full bg-blue-300"/>
                  })()}
                  <h3 className="text-xl md:text-2xl lg:text-2xl font-bold">{d.title}</h3>
                  <p className="text-sm md:text-lg  ">{d.desc}</p>
                </motion.div>
            ))}
         </div>
        </section>
